...
//                if(!isValid(world, source.getX(), source.getZ())) return;
	...
	private void buildAirAndWaterfall(final Set<Block> ground, final Set<Block> blocks, final Random random, final World world) {
		...
		if (!candidates.isEmpty()) {
			buildWaterfall(candidates.get(random.nextInt(candidates.size())), world);
	...
	private Block buildLake(Set<Block> top_layer, final Random random, final World world) {
                final int water_type = random.nextInt(10);
		...
		for (final Block block : top_layer) {
			block.setType(Material.WATER);
                        if(water_type != 0) world.setBiome(block.getX(), block.getZ(), Biome.RIVER);
                        else world.setBiome(block.getX(), block.getZ(), Biome.COLD_OCEAN);
		}
...
//					candidate.setType(Material.GLOWSTONE);
	...
	private void buildWaterfall(final Block block, final World world) {
		final BlockFace[] faces = {BlockFace.NORTH, BlockFace.EAST, BlockFace.SOUTH, BlockFace.WEST};
		for (final BlockFace f : faces) {
			final Block r = block.getRelative(f);
			if (!r.isEmpty()) {
				r.setType(Material.WATER);
                                world.setBiome(r.getX(), r.getZ(), Biome.RIVER);
				return;
			}
		}
	}
